...
async function probeAudio(file) {
  const args = ['-v', 'error', '-of', 'json', '-show_entries', 'stream=sample_rate,channels,bit_rate', '-select_streams', 'a:0', file];
  try {
...
app.post('/merge', async (req, res) => {
  ...
  const { bookId, chapterAudioUrls, callbackUrl, chapterTitles } = req.body || {};
  if (!bookId || !Array.isArray(chapterAudioUrls) || chapterAudioUrls.length === 0 || !callbackUrl) {
    return res.status(400).json({ error: 'bookId, chapterAudioUrls[], callbackUrl required' });
  }

  // Respond queued quickly; continue work in background
  ...
  (async () => {
    ...
    try {
      ...
      await run(ffmpegBin(), ['-i', outPath, '-i', ffmetaPath, '-map_metadata', '1', '-codec', 'copy', '-write_id3v2', '1', '-id3v2_version', '3', outWithChapters]);
      ...
      // Callback error
      await fetch((req.body && req.body.callbackUrl) || '', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bookId: (req.body && req.body.bookId) || 'unknown', error: String(err && err.message || err) })
      }).catch(() => {});
    } finally {
...
app.listen(port, () => console.log('merge worker listening on', port));